...
```
daml build
daml codegen js .daml/dist/MyThirdDamlApp-0.1.0.dar -o ui/daml.js
```
...
Deploying `MyThirdDamlApp` to the hosted Daml platform
...
```
daml build -o MyThirdDamlApp.dar
```
...
DAR `MyThirdDamlApp.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/MyThirdDamlApp-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../MyThirdDamlApp-ui.zip build)
```
...
(`MyThirdDamlApp-ui.zip`) you have just created and publish it.

To see your deployed instance of `MyThirdDamlApp` in action, follow the
...
- `daml.js/MyThirdDamlApp-0.1.0/src/User.ts` (once you've generated it):
