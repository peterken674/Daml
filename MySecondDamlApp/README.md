...
Deploying `MySecondDamlApp` to the hosted Daml platform
...
```
daml build -o MySecondDamlApp.dar
```
...
DAR `MySecondDamlApp.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/MySecondDamlApp-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../MySecondDamlApp-ui.zip build)
```
...
(`MySecondDamlApp-ui.zip`) you have just created and publish it.

To see your deployed instance of `MySecondDamlApp` in action, follow the
...
- `daml.js/MySecondDamlApp-0.1.0/src/User.ts` (once you've generated it):
